...
function cleanTextForGroq(text: string): string {
  if (!text) return '';
  
  // Remplacer les caractères Unicode problématiques
  return text
    .replace(/•/g, '-') // Remplacer les bullet points (char 8226) par des tirets
    .replace(/[”“"‘’'«»]/g, '"') // Normaliser les guillemets
    .replace(/[–—]/g, '-') // Normaliser les tirets
    .replace(/[^\x00-\xFF]/g, '') // Garder seulement les caractères ASCII étendu
...
async function generateWithGroq(query: string, context: string, groqKey: string): Promise<string> {
  ...
  // Nettoyer le contexte et la requête
  const cleanContext = cleanTextForGroq(context)
  const cleanQuery = cleanTextForGroq(query)
...
export async function POST(request: Request) {
  try {
    ...
    // Build context from retrieved documents and clean it
    const rawContext = retrievedDocs.map((doc, index) => 
      `INCIDENT ${index + 1}:
Title: ${doc.metadata.title}
Date: ${doc.metadata.date}
Source: ${doc.metadata.source}
...
Content: ${doc.content}`
    ...
    // Nettoyer le contexte avant de l'envoyer à Groq
    const cleanContext = cleanTextForGroq(rawContext)

    // Generate answer using Groq API with cleaned context
    const answer = await generateWithGroq(query, cleanContext, groqKey)
